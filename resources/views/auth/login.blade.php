    ...
    <form class="mt-8" action="/login" method="POST">
      ...
      <div class="rounded-md shadow-sm">
        ...
        <div class="mt-5 relative">
          <input aria-label="Password" name="password" type="password" required class="appearance-none rounded relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:shadow-outline-indigo focus:border-indigo-600 focus:z-10 sm:text-sm sm:leading-5 @error('password') border-red-500 @enderror" placeholder="Password" />
          @error('password')
            <span class="absolute text-red-500 text-xs">{{$message}}</span>
          @enderror
        </div>
